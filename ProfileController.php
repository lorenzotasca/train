<?php

    session_start();

    include "connection.php";

    // convert cm into ft
    function cm_to_ft($cm) {
        // 1 piede = 30.48 centimetri
        return $cm / 30.48;
    }
    
    $standing_reach_ft = cm_to_ft($_POST['standing_reach_cm']);
    $height_wo_shoes_ft = cm_to_ft($_POST['height_wo_shoes_cm']);
    $wingspan_ft = cm_to_ft($_POST['wingspan_cm']);



    // Prepare the SQL statement
    $stmt = $connessione->prepare("INSERT INTO Giocatore (ruolo, standing_reach, altezza_senza_scarpe, apertura_alare) VALUES (?, ?, ?, ?)");

    // Bind the parameters
    $stmt->bind_param("ssss", $_POST['role'], $standing_reach_ft, $height_wo_shoes_ft, $wingspan_ft);

    // Try to execute the statement and handle any errors
    try {
        $stmt->execute();
        //header("Location: Login.php");
    } catch (Exception $e) {
        $err = $e->getMessage();
        header("Location: Profile.php?err=$err");
    } finally {
        // Always close the statement and connection, even if an error occurred
        $stmt->close();
        $connessione->close();
    }

?>